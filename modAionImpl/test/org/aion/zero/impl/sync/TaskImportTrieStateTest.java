...
/**
 * Unit tests for {@link TaskImportTrieState}.
 *
 * @author Alexandra Roatis
 */
@RunWith(JUnitParamsRunner.class)
public class TaskImportTrieStateTest {
    ...
    public void testRun_shutdown() {
        ...
        FastSyncMgr fastSyncMgr = mock(FastSyncMgr.class);
        when(fastSyncMgr.isComplete()).thenReturn(true);

        BlockingQueue<TrieNodeWrapper> trieNodes = mock(LinkedBlockingQueue.class);

        // run task
        TaskImportTrieState task = new TaskImportTrieState(log, chain, trieNodes, fastSyncMgr);
        task.run();
