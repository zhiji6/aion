...
public class AionPendingStateImpl implements IPendingState {
    ...
    private TxBroadcastCallback txBroadcastCallback = null;
    ...
    public static AionPendingStateImpl create(
        ...
        return new AionPendingStateImpl(
                blockchain,
                energyUpperBound,
                txPendingTimeout,
                maxTxCacheSize,
    ...
    public static AionPendingStateImpl createForTest(
            AionBlockchainImpl blockchain,
            long energyUpperBound,
            int txPendingTimeout,
            int maxTxCacheSize,
        ...
        AionPendingStateImpl ps =
                new AionPendingStateImpl(
                        ...
                        poolDump);
        ps.testingMode = true;
        ps.poolDumpEnable = !seedMode;
        return ps;
    }

    private AionPendingStateImpl(
            AionBlockchainImpl blockchain,
            long energyUpperBound,
            int txPendingTimeout,
            int maxTxCacheSize,
    ...
    public synchronized TxResponse addTransactionFromApiServer(AionTransaction tx) {
        ...
        if (isSeedMode || !closeToNetworkBest) {
            txBroadcastCallback.broadcastTx(Collections.singletonList(tx));
    ...
    public synchronized void addTransactionsFromNetwork(List<AionTransaction> transactions) {
        ...
        // SeedMode or the syncing status will just broadcast the transaction to the network.
        if (isSeedMode || !closeToNetworkBest) {
            txBroadcastCallback.broadcastTx(validTransactions);
        } else {
    ...
    private List<TxResponse> addPendingTransactions(
        ...
        if (!newPending.isEmpty()) {
            if (pendingTxCallback != null) {
                pendingTxCallback.pendingTxReceivedCallback(newPending);
            }

            if (pendingTxReceivedforMining != null) {
     ...
     */
    @Override
    public synchronized void flushPendingState(Block newBlock, List<AionTxReceipt> receipts) {
    ...
    @Override
    public void setNewPendingReceiveForMining(AtomicBoolean newPendingTxReceived) {
        pendingTxReceivedforMining = newPendingTxReceived;
    }

    @Override
    public void setTxBroadcastCallback(TxBroadcastCallback txBroadcastCallback) {
        this.txBroadcastCallback = txBroadcastCallback;
    }
